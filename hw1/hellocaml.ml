...
let centimes_of : int -> int -> int =
  fun (rp : int) (fr : int) : int -> rp + fr  * 100
...
(*
  ...
  implements many useful functions for list manipulation.  You will
  ...
  begin match e with
    | Add (a, b) -> begin
    	...
    	begin match a, b with
    	  | Const c0, Const c1 -> Const (Int64.add c0 c1)
    	  | Const 0L, x
    	  | x, Const 0L -> x
    	  | x, Neg y when x = y -> Const 0L
    	  | Neg x, y when x = y -> Const 0L
        | x, y -> Add (x, y)
    	end
    end
    | Mult (a, b) -> begin
      ...
      let b = optimize b in
      begin match a, b with
        | Const c0, Const c1 -> Const (Int64.mul c0 c1)
        | Const 0L, x
        | x, Const 0L -> Const 0L
        | Const 1L, x
        | x, Const 1L -> x
        | Neg x, Neg y when x = y -> Mult (x, y)
        | x, y -> Mult (x, y)
      end
    end
    | Neg a -> begin
