...
module IGraph =
  struct
    ...
    type t = {
      i:      UidSet.t UidMap.t  (* interference graph: uid -> {adjacent uids} *)
    ; i_keys: UidSet.t
    ; p:      Alloc.loc UidMap.t (* preference: uid -> loc *)
    ; p_keys: UidSet.t
    }
...
(*      Platform.verb @@ Printf.sprintf "edge: %s -> %s\n" u v;*)
    ...
    (* Get edge count of a node *)
    let edge_count (g: t) (u: Ll.uid) : int =
      UidSet.cardinal (UidMap.find u g.i)

    (* Remove a node from the interference graph *)
    let remove_node (g: t) (u: Ll.uid) : t =
      let u_s = UidMap.find u g.i in
      (* Remove edge from v to u *)
      let remove_reversed_edge v i =
        let v_s = UidMap.find v i in
        let v_s' = UidSet.remove u v_s in
        UidMap.add v v_s' i
      in
      (* Remove in-edges of u *)
      let i' = UidSet.fold remove_reversed_edge u_s g.i in
      (* Remove out-edges of u*)
      {g with i = (UidMap.remove u i')
            ; i_keys = (UidSet.remove u g.i_keys)}

    (* Add a preference *)
    let add_preference (g: t) (u: Ll.uid) (loc: Alloc.loc) : t =
      {g with p = (UidMap.add u loc g.p)
            ; p_keys = (UidSet.add u g.p_keys)}

    (* Get preference *)
    let get_preference_opt (g: t) (u: Ll.uid) : Alloc.loc option =
      UidMap.find_opt u g.p
  ...
  *)
  let rec allocate (g: IGraph.t) : (uid * Alloc.loc) list =
    if (UidSet.is_empty g.i_keys) then !base_lo (* done, using !base_lo as base *)
      ...
      let choose_node (s: UidSet.t) : uid =
        try UidSet.find_first (fun u -> (IGraph.edge_count g u) < pal_size) s (* find a node with < K edges *) with
        | Not_found -> UidSet.choose s  (* not found, randomly choose one *)
      in

      let n =
        (* Try to find a node without preference *)
        try choose_node (UidSet.diff g.i_keys g.p_keys) with
        (* Find a node with preference *)
        | Not_found -> choose_node (UidSet.inter g.i_keys g.p_keys)
      in

      let g' = IGraph.remove_node g n in  (* remove/freeze the node *)
      let lo = allocate g' in  (* color the remaining graph *)
